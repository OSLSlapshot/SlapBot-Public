...
const getPlayerFromDiscordIdQueryMoreInfo = async (discordId) => {
	...
	if (playerIdx !== -1) {
		const PlayerPath = currDataPath+`/PlayerData/${PlayerData[playerIdx][0]} ${PlayerData[playerIdx][2]}/`;
		
		const SelPlayerRatingsData = readDatabase(PlayerPath + '/RatingChanges.txt');
		
		//casual RPUGs
		const SelPlayerCasualRatingsData = SelPlayerRatingsData.filter(row => row[6] === 'casual');
		...
		if (SelPlayerCasualRatingsData.length !== 0) {
			...
			//rating extrema
			const CasualRatingValues = SelPlayerCasualRatingsData.map(row => parseInt(row[3]) + parseInt(row[4]));
			...
			//latest streak
			const lastSign = CasualRatingChangesCol[CasualRatingChangesCol.length - 1][0] //starting sign to check
			let count = 0;
			...
			for (const changeVal of CasualRatingChangesCol) {
				if (parseInt(changeVal) === 0) {} //just incase there is a 0 rating change
					...
					else if (prevSign === '-') { //explicitly checking just incase
			...
			*/
			highestCLStreak = highestCLStreak === 0 ? '-' : '-' + highestCLStreak.toString();
			highestCWStreak = highestCWStreak === 0 ? '-' : '+' + highestCWStreak.toString();
		}

		//twos RPUGs
		const SelPlayerTwosRatingsData = SelPlayerRatingsData.filter(row => row[6] === 'twos');
		...
		if (SelPlayerTwosRatingsData.length !== 0) {
			...
			//rating extrema
			const TwosRatingValues = SelPlayerTwosRatingsData.map(row => parseInt(row[3]) + parseInt(row[4]));
			...
			highestTLStreak = highestTLStreak === 0 ? '-' : '-' + highestTLStreak.toString();
			highestTWStreak = highestTWStreak === 0 ? '-' : '+' + highestTWStreak.toString();
		}
...
const getPlayerCareerFromDiscordIdQuery = async (discordId) => {
	...
	for (const dir of datesDirs) {
		...
		if (playerIdx !== -1) {
			carCasualWins += parseInt(PlayerData[playerIdx][7]);
			carCasualLosses += parseInt(PlayerData[playerIdx][8]);
			carTwosWins += parseInt(PlayerData[playerIdx][9]);
			carTwosLosses += parseInt(PlayerData[playerIdx][10]);
			
			if (parseFloat(PlayerData[playerIdx][4])!== cfg.trueskill.casualInitTS.initialSigma) { //checking to see if any matches played
				carCasualEOSRatings.push(parseInt(PlayerData[playerIdx][3])); //EOS rating
				
				const PlayerCasualRatingChanges = RatingsData.filter(row => ((row[2] === PlayerData[playerIdx][0]) && ((row[6] === 'casual') || (row[6] === 'open')))); //filter Ratings for selected player and casual/open mode
				const PlayerCasualRatingValues = PlayerCasualRatingChanges.map(row => parseInt(row[3]) + parseInt(row[4]));
				...
				if (PlayerCasualRatingValues.length > 0) {
					...
					CasualRatingCol = CasualRatingCol.map(r => parseInt(r));
					CasualRatingCol.push(CasualRatingCol[(CasualRatingCol.length - 1)] + parseInt(PlayerCasualRatingChanges[(CasualRatingCol.length - 1)][4])); //add latest rating to the array
			...
			if ((parseFloat(PlayerData[playerIdx][6])!== cfg.trueskill.twosInitTS.initialSigma) && (parseFloat(PlayerData[playerIdx][6])!== cfg.trueskill.twosInitTS.initialSigma/2)) {
				...
				const PlayerTwosRatingChanges = RatingsData.filter(row => ((row[2] === PlayerData[playerIdx][0]) && (row[6] === 'twos'))); //filter Ratings for selected player and twos mode
				const PlayerTwosRatingValues = PlayerTwosRatingChanges.map(row => parseInt(row[3]) + parseInt(row[4]));
				...
				if (PlayerTwosRatingValues.length > 0) {
					...
					TwosRatingCol = TwosRatingCol.map(r => parseInt(r));
					TwosRatingCol.push(TwosRatingCol[(TwosRatingCol.length - 1)] + parseInt(PlayerTwosRatingChanges[(TwosRatingCol.length - 1)][4])); //add latest rating to the array
	...
	if ((carCasualWins+carCasualLosses+carTwosWins+carTwosLosses) === 0) {
		return null;
	}
	...
	carCasualRating.avg = carCasualEOSRatings.reduce((a,b) => a + b, 0) / carCasualEOSRatings.length
	...
	carTwosRating.avg = carTwosEOSRatings.reduce((a,b) => a + b, 0) / carTwosEOSRatings.length
	
	//Casual Rating Worm
	const xAxisLength = allCasualMatchRatings.length > allTwosMatchRatings.length ? (Math.ceil(allCasualMatchRatings.length/5)*5) : (Math.ceil(allTwosMatchRatings.length/5)*5);
	const width = 1000;
	const height = 400;
...
const getVersusInfo = async (discordId,username) => {
	...
	const player2Idx = PlayerData.map(x => stripUsername(x[2]).toLowerCase()).indexOf(stripUsername(username.toString()).toLowerCase());	//player 2 for versus info
	
	let player1 = {};
	let player2 = {};
	if ((playerIdx !== -1) && (player2Idx !== -1)) { //if both players found in database
		player1 = new Player(PlayerData[playerIdx][0],PlayerData[playerIdx][1],PlayerData[playerIdx][2],parseInt(PlayerData[playerIdx][3]),parseFloat(PlayerData[playerIdx][4]),parseInt(PlayerData[playerIdx][5]),parseFloat(PlayerData[playerIdx][6]),parseInt(PlayerData[playerIdx][7]),parseInt(PlayerData[playerIdx][8]),parseInt(PlayerData[playerIdx][9]),parseInt(PlayerData[playerIdx][10]),PlayerData[playerIdx][11],PlayerData[playerIdx][12]);
		player2 = new Player(PlayerData[player2Idx][0],PlayerData[player2Idx][1],PlayerData[player2Idx][2],parseInt(PlayerData[player2Idx][3]),parseFloat(PlayerData[player2Idx][4]),parseInt(PlayerData[player2Idx][5]),parseFloat(PlayerData[player2Idx][6]),parseInt(PlayerData[player2Idx][7]),parseInt(PlayerData[player2Idx][8]),parseInt(PlayerData[player2Idx][9]),parseInt(PlayerData[player2Idx][10]),PlayerData[player2Idx][11],PlayerData[player2Idx][12]);
	}
	else {
		return null;
	}
	...
	const seasonVsData = MatchesData.filter(row => (row[2].split(',').includes(player1.playerID)) && (row[2].split(',').includes(player2.playerID)));
...
const getVersusCareerInfo = async (discordId,username) => {
	...
	for (const dir of datesDirs) {
		...
		const seasonVsData = MatchesData.filter(row => (row[2].split(',').includes(player1.playerID)) && (row[2].split(',').includes(player2.playerID)));
		const seasonVsDataCasual = seasonVsData.filter(row => ((row[5] === 'casual') || (row[5] === 'open')));
		const seasonVsDataTwos = seasonVsData.filter(row => row[5] === 'twos');
...
const getPie = async (mode) => {
	...
	let player1 = {};
	if (playerIdx !== -1) { //if both players found in database
		player1 = new Player(PlayerData[playerIdx][0],PlayerData[playerIdx][1],PlayerData[playerIdx][2],parseInt(PlayerData[playerIdx][3]),parseFloat(PlayerData[playerIdx][4]),parseInt(PlayerData[playerIdx][5]),parseFloat(PlayerData[playerIdx][6]),parseInt(PlayerData[playerIdx][7]),parseInt(PlayerData[playerIdx][8]),parseInt(PlayerData[playerIdx][9]),parseInt(PlayerData[playerIdx][10]),PlayerData[playerIdx][11],PlayerData[playerIdx][12]);
	}
	else {
		return null;
	...
	const MatchesData = readDatabase(currDataPath+'/Matches.txt');
	const filteredMatches = MatchesData.filter(row => ((row[2].split(',').includes(player1.playerID)) && (row[5] === mode)));
	...
	for (const match of filteredMatches) {
		...
		for (const p of playerIDs) {
			if (p !== player1.playerID) {
				...
				if (notFound) {
					const pIdx = PlayerData.map(x => x[1]).indexOf(discordId.toString()); //player running the command
					matchPlayers.push({playerID: p, username: PlayerData[pIdx][2], stats: {WinsWith: 0, WinsAgainst: 0, LossesWith: 0, LossesAgainst: 0}});
				}
			}
		}
		//
...
const getSeasonSummary = async () => {
	...
	const CasualPlayerData = PlayerData.filter(row => row[4] !== cfg.trueskill.casualInitTS.initialSigma.toString()); //filter all players who have not played a casual match
	const CasualRatings = getCol(CasualPlayerData,3);	//casual ratings col
	const TwosPlayerData = PlayerData.filter(row => row[6] !== cfg.trueskill.twosInitTS.initialSigma.toString()); //filter all players who have not played a twos match
	const TwosRatings = getCol(TwosPlayerData,5);	//twos ratings col
	
	const x_vals = [850, 950, 1050, 1150, 1250, 1350, 1450, 1550, 1650, 1750, 1850, 1950, 2050, 2150, 2250, 2350];
	...
	const barBackgroundColorCasual = Array(x_vals.length).fill('rgba(255, 255, 0, 0.6)');
	const barBorderColorCasual = Array(x_vals.length).fill('rgba(255, 255, 0, 1)');
	
	const barBackgroundColorTwos = Array(x_vals.length).fill('rgba(0, 255, 255, 0.6)');
...
const seasonRollQuery = async () => {
	...
	const activePlayers = PlayerData.filter(row => (parseInt(row[7]) + parseInt(row[8]) + parseInt(row[9]) + parseInt(row[10])) !== 0); //Players with atleast 1 match
	...
	*/
	...
	while (!drawWinner) {
		...
		const groupNum = (groupRoll < 0.02) ? 0 : (groupRoll < 0.15) ? 1 : (groupRoll < 0.4) ? 2 : 3;
		//const selPlayers = activePlayers.filter(row => (parseInt(row[7]) + parseInt(row[8]) + parseInt(row[9]) + parseInt(row[10])));
		let selPlayers = [];
		for (const p of activePlayers) {
			const pMatches = parseInt(p[7]) + parseInt(p[8]) + parseInt(p[9]) + parseInt(p[10]);
			if (pMatches >= matchBrackets[groupNum].min) {
				if (matchBrackets[groupNum].max) {	//not matchBracket 3
	...
	drawWinner = new Player(drawWinner[0],drawWinner[1],drawWinner[2],parseInt(drawWinner[3]),parseFloat(drawWinner[4]),parseInt(drawWinner[5]),parseFloat(drawWinner[6]),parseInt(drawWinner[7]),parseInt(drawWinner[8]),parseInt(drawWinner[9]),parseInt(drawWinner[10]),drawWinner[11],drawWinner[12]);
