...
class CosmeticManager {
	...
	async getTypePageGraphic(cosmeticList) {
		...
		const canvas = new Canvas(img_colcount*225+img_margin,img_rowcount*275+img_margin);	//must maintain ratio above ~1.35 for widest embed (with no thumbnail)
		...
		for (let i = 0; i < img_rowcount; i++) {
			for (let j = 0; j < img_colcount; j++) {
				...
				try {
					ctx.drawImage(previews[cos_idx],225*j+25,275*i+25);
					ctx.textAlign = 'center';
					ctx.textBaseline = 'top';
					ctx.fillStyle = 'white';
					ctx.font = "50px Arial";
					ctx.fillText(`[${cos_idx+1}]`,225*j+125,275*i+225);
				}
				catch {
					continue;
				}
