...
async function refreshTeamsList() {
	...
	for (const team of leagueTeams) {
		const leagueListTeamNameColumnIdx = getColNameIdx(LeagueListDataHeader,['Team'])
		const teamPlayerList = LeagueListData.filter((x) => x[leagueListTeamNameColumnIdx] === team.teamName).map((x) => x[getColNameIdx(LeagueListDataHeader,['DiscordID'])]);	//must use discord IDs instead of player IDs because players on the team may not be registered in RPUGs- using PIDs would unnecessary complexity to the process
...
async function writeSelectList() {
	...
	const sheet = await importGSheet(cfg.rpugsSheet.sheetNames.selectList);
	const rows = await sheet.getRows();
...
async function updateCosmeticLibraryQuery(cosmetics) {
	const cosmetic_dtb_path = databasePath + '/cosmetics/cosmetics.json';
