...
import { getPublicMatchmaking } from '../queries/slapshot';
//import { isCommissioner } from '../utls/isMod';
import { bot, server, cmdChannels } from '../index';
import { getQueues } from '../commands/queue';
import { getOngoingDrafts } from './draftClass';
...
async function updateSlapQueueTextChannel(firstCall = false) {
	...
	if (firstCall) {
		bot.user.setActivity(`Oce Pubs: ${playerCount}`,{type: "WATCHING"}).catch(console.error);
	}
	...
	if (msgObj) {
		var sentMsg = await msgObj.edit({ files: embedFilesList, embed: QCEmbed}).catch(console.error);
	}
