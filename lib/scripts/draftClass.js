...
class Draft {
    ...
    async startDraft() {
		...
		.then( emMsg => {
			...
			let checkInMsgTimer = null;

			//collector.once('collect', (reaction,user) => {});
			collector.on('collect', async i => {
				if (checkInMsgTimer) {
					clearTimeout(checkInMsgTimer);
					checkInMsgTimer = null;
				}
				...
				checkInMsgTimer = setTimeout(async () => {
					for (const r of i.message.components) {
						for (const b of r.components) {
							if (!poppedQueue.includes(b.data.custom_id.split(' ')[1])) {	//only checking players who haven't checked in
								b.data.disabled = true;
							}
						}
					}
        ...
        if (this.draftNumber < ( this.draftConfig.numPlayers - 1 )) {
			...
			if (this.draftNumber == 1) {
					...
					for (let c = 0; c < 4; c++) {
						...
						row.addComponents(
							new Discord.ButtonBuilder()
								...
								.setLabel(`${playerNum}. ${currPlayer.username} (` + currPlayer[`${this.mode}`+'Rating'] + ')')
								.setStyle(Discord.ButtonStyle.Success)
						);
				...
				*/
				...
				const currDraftMsg = await currentCaptainClient.send({ components: buttonRows, files: embedFilesList, embeds: [init1CEmbed]}).catch(console.error);
				...
				*/
				...
				const otherDraftMsg = await otherCaptainClient.send({ components: disabled_buttons, files: embedFilesList, embeds: [init2CEmbed]}).catch(console.error);
				//break;
           ...
           // await this.captainsClient.teamB.send({ files: embedFilesList, embeds: [DCEmbed]}).catch(console.error);
			this.captainDraftMsgs.embeds[this.currentCaptain] = await this.captainDraftMsgs.embeds[this.currentCaptain].edit({ files: embedFilesList, embeds: [DCEmbed]}).catch(console.error);
			this.captainDraftMsgs.embeds[this.otherCaptain] = await this.captainDraftMsgs.embeds[this.otherCaptain].edit({ files: embedFilesList, embeds: [DCEmbed]}).catch(console.error);
			for (const player of this.nonCaptains) {
                const playerClient = await bot.users.fetch(player.discordId);
        ...
        for (const [currentIndex, player] of Object.entries(this.remainingPlayers)) {
            if (currentIndex === num) {
				...
				if (this.draftNumber < this.draftLength) {
					const nextDraftCaptain =  this.draftOrder[this.draftNumber + 1];
			...
			const filter =  (reaction, user) => {
				if ((reaction.emoji.name === '✅') && ((this.teamA.playerList.map(x => x.discordID)).includes(user.id)) && !((this.teamACheckedIn.map(x => x.discordID)).includes(user.id))) {
					...
					if ((getDraftFromDiscordId(user.id) !== false) && (user.id !== this.captainsObject[0].discordId)) {
						...
						cmdChannels.commandsCh.send({ files: errObj.embedFiles, embeds: [errObj.embedMessage]}).catch(console.error);
					}
...
async function removeDraft(userDraftIndex,cancelObj) {
    for (const [arrayIndex, draft] of ongoingDrafts.entries()) {
        if (draft.index === Number(userDraftIndex)) {
			...
			while (!draft.checkinCollector) {
				await sleep(500);
				waited++;
			}
			while (!draft.captainDraftMsgs) {
				await sleep(500);
				waited++;
			}
			...
			if (draft.mode !== 'scrims') {
				if (draft.draftCompleteCollector) {
					draft.draftCompleteCollector.stop();
				}
				if ((draft.captainDraftMsgs) && (draft.captainDraftMsgs.collectors)) {
					for (const c of Object.values(draft.captainDraftMsgs.collectors)) {
						c.stop();
					}
					for (const m of Object.values(draft.captainDraftMsgs.embeds)) {
						await m.edit({
							components:  draft.captainDraftMsgs.buttons.disabled_buttons
						});
					}
				}
			}
