...
async function updatePlayerRatings(match,teamA, teamB, winner, mode) {
	...
	if (mode === 'scrims') {
		...
		for (const member of teamA) {
			if (mode === 'twos') {
				previousRatingsA.push(new Rating(member.twosRating / 100, member.twosSigma));
				ratingsChange[member.discordId] = {
					previousRating: member.twosRating,
					previousSigma: member.twosSigma
				};
			} else {
				previousRatingsA.push(new Rating(member.casualRating / 100, member.casualSigma));
				ratingsChange[member.discordId] = {
		...
		for (const member of teamB) {
			if (mode === 'twos') {
				previousRatingsB.push(new Rating(member.twosRating / 100, member.twosSigma));
				ratingsChange[member.discordId] = {
					previousRating: member.twosRating,
					previousSigma: member.twosSigma
				};
			} else {
				previousRatingsB.push(new Rating(member.casualRating / 100, member.casualSigma));
				ratingsChange[member.discordId] = {
					previousRating: member.casualRating,
					previousSigma: member.casualSigma
				};
	...
	if (mode === 'scrims') {
		...
		const response = await updatePlayerRatingsQuery(match,preparedRatings, mode);
	}
