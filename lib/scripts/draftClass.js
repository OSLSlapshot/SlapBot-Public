		...
		if (this.draftNumber === 1) {
			...
			*/
			...
			try {
				...
				if (e.code === 50007) {
					const errObj = errorMsg('Unable to message the following captain:',`<@${currentCaptainClient.id}>/${currentCaptainClient.username}\nPlease enable permissions for <@${bot.user.id}> to direct message you.\nMatch cancelled.`,null,false);
					await cmdChannels.updatesCh.send({ files: errObj.embedFiles, embeds: [errObj.embedMessage]}).catch(console.error);
			...
			*/
			...
			try {
				...
				if (e.code === 50007) {
					const errObj = errorMsg('Unable to message the following user:',`<@${otherCaptainClient.id}>/${otherCaptainClient.username}\nPlease enable permissions for <@${bot.user.id}> to direct message you.\nMatch cancelled.`,null,false);
					await cmdChannels.updatesCh.send({ files: errObj.embedFiles, embeds: [errObj.embedMessage]}).catch(console.error);
