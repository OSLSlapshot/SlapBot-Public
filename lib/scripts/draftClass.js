...
import { getQueues } from '../commands/queue';
import { createLobby } from '../queries/slapshot';
...
class Draft {
    ...
    async startDraft() {
		...
		let embedFilesList = [];
		const embedThumb = new Discord.MessageAttachment('./thumbnails/alert.png', 'alert.png'); //from: created on MS Word
		...
		for (const player of this.nonCaptains) {
			...
			await playerClient.send({ files: embedFilesList, embed: initNCEmbed}).catch(console.error);
		}
		...
		*/
		...
		await cmdChannels.updatesCh.send({ files: embedFilesList, embed: initNCEmbed})
		.then( emMsg => {
			emMsg.react('✅');
			...
			collector.on('end', async function(collected,reason) {
				if (reason === 'time') {	//limit, time, userLimit
					...
					if (cancelDraftIdx) {
						cmdChannels.modCh.send({embed: ACEmbed}).catch(console.error);
					...
					cmdChannels.updatesCh.send({ embed: ACEmbed });
					...
					if (waited === 300*2) {
						...
						cmdChannels.updatesCh.send({ embed: WaitEmbed });
					}
					...
					if (thisClass.gamepass) {
						...
						cmdChannels.updatesCh.send({ embed: resultEmbed });
					}
				}
			});
		})
		.catch(err => console.error(err));
			...
			if (this.notCheckedIn.length > 0) {
				...
				cmdChannels[this.mode + 'Ch'].send({ embed: ACEmbed });
			}
		})
		.catch(console.error);
		*/
    ...
    async nextDraft(draftedPlayer = null) {
        if (this.draftNumber < 5) {
            ...
            switch (this.draftNumber) {
				case 1:
					let embedFilesList = [];
					const embedAuthThumb = new Discord.MessageAttachment('./thumbnails/alert.png', 'alert.png'); //from: created on MS Word
					embedFilesList.push(embedAuthThumb);

					let embedThumb = new Discord.MessageAttachment('./thumbnails/turnToDraftTrue.png', 'turnToDraftTrue.png'); //from: created on MS Word
					embedFilesList.push(embedThumb);
					...
					await currentCaptainClient.send({ files: embedFilesList, embed: init1CEmbed}).catch(console.error);
					
					
					embedFilesList = [];
					embedFilesList.push(embedAuthThumb);
					
					embedThumb = new Discord.MessageAttachment('./thumbnails/turnToDraftFalse.png', 'turnToDraftFalse.png'); //from: created on MS Word
					embedFilesList.push(embedThumb);
					...
					await otherCaptainClient.send({ files: embedFilesList, embed: init2CEmbed}).catch(console.error);
					break;

				case 2:
					embedFilesList = [];
					embedThumb = new Discord.MessageAttachment('./thumbnails/turnToDraftTrue.png', 'turnToDraftTrue.png'); //from: created on MS Word
					embedFilesList.push(embedThumb);
					...
					await currentCaptainClient.send({ files: embedFilesList, embed: draft2Embed}).catch(console.error);
					...
					await otherCaptainClient.send({ files: embedFilesList, embed: draft2Embed}).catch(console.error);
					break;
				...
				case 3:
					...
					let draft3Embed = {
						...
						fields: [
							...
							},
							{
								...
								value: 'Please type **!d  <number>** to draft the corresponding player to your team.'
							}
						]
					};
					
					await currentCaptainClient.send({ files: embedFilesList, embed: draft3Embed}).catch(console.error);
					...
					await otherCaptainClient.send({ files: embedFilesList, embed: draft3Embed}).catch(console.error);
					break;
			...
			let embedFilesList = [];
			const embedThumb = new Discord.MessageAttachment('./thumbnails/draftComplete.png', 'draftComplete.png'); //from: 
			embedFilesList.push(embedThumb);
			
			const embedThumb2 = new Discord.MessageAttachment('./thumbnails/casualArena.jpg', 'casualArena.jpg'); //from: 
			...
			await this.captainsClient.teamA.send({ files: embedFilesList, embed: DCEmbed}).catch(console.error);
            await this.captainsClient.teamB.send({ files: embedFilesList, embed: DCEmbed}).catch(console.error);
			for (const player of this.nonCaptains) {
                const playerClient = await bot.users.fetch(player.discordId);
                await playerClient.send({ files: embedFilesList, embed: DCEmbed}).catch(console.error);
            }
			...
			*/
			await cmdChannels.updatesCh.send({ files: embedFilesList, embed: DCEmbed}).catch(console.error);
        }
    }
...
class TwosDraft {
    ...
    async startDraft() {
		...
		*/
		...
		.then( emMsg => {
			...
			const filter =  (reaction, user) => reaction.emoji.name === '✅' && poppedQueue.includes(user.id);
			const collector = emMsg.createReactionCollector(filter, { maxUsers: 4, time: cfg.checkinTime * 60 * 1000 }); // 5 min
    ...
    async nextDraft(draftedPlayer = null) {
        if (this.draftNumber < 3) {
			...
			const embedThumb2 = new Discord.MessageAttachment('./thumbnails/twosArena.jpg', 'twosArena.jpg'); //from: my own
			embedFilesList.push(embedThumb2);
...
class ScrimsDraft {
    ...
    async startCheckin() {
		...
		for (const player of this.teamA.playerList) {
			const playerClient = await bot.users.fetch(player.discordID);
			await playerClient.send({ files: embedFilesList, embed: initNCEmbed}).catch(async function (error) {
				if (error.code === 50007) {
		...
		*/
		...
		.then( emMsg => {
			...
			const filter =  (reaction, user) => {
				if ((reaction.emoji.name === '✅') && ((this.teamA.playerList.map(x => x.discordID)).includes(user.id)) && !((this.teamACheckedIn.map(x => x.discordID)).includes(user.id))) {
					...
					if ((getDraftFromDiscordId(user.id) !== false) && (user.id !== this.captainsObject[0].discordId)) {
						...
						cmdChannels.scrimsCh.send({ files: errObj.embedFiles, embed: errObj.embedMessage}).catch(console.error);
					}
					else if (((queues.casual.map(player => player.discordId)).includes(user.id)) || ((queues.twos.map(player => player.discordId)).includes(user.id))) {
			...
			const collector = emMsg.createReactionCollector(filter, { maxUsers: 1, time: cfg.checkinTime * 60 * 1000 }); // 5 min
	...
	async checkinComplete() {
		...
		for (const player of this.teamACheckedIn) {
			const playerClient = await bot.users.fetch(player.discordID);
			await playerClient.send({ files: embedFilesList, embed: DCEmbed});
		}
    ...
    for (const [arrayIndex, draft] of ongoingDrafts.entries()) {
        if (draft.index === Number(userDraftIndex)) {
			if (cancelObj.cancelCalled) {
				...
				let embedFilesList = [];
				const embedThumb = new Discord.MessageAttachment('./thumbnails/cancel.png', 'cancel.png'); //from: https://www.pngkit.com/png/detail/99-993265_bar-graph-clip-art.png
				embedFilesList.push(embedThumb);
				...
				if (draft.mode == 'scrims') {
					await draft.captainsClient.teamA.send({ files: embedFilesList, embed: cancelDMEmbed});
					await draft.captainsClient.teamB.send({ files: embedFilesList, embed: cancelDMEmbed});
					for (const player of draft.teamACheckedIn) {
						if (player.discordID !== draft.captainsObject[0].discordId) {
							const playerClient = await bot.users.fetch(player.discordID);
							await playerClient.send({ files: embedFilesList, embed: cancelDMEmbed});
						}
					}
				...
				if (draft.mode === 'casual') {
					await cmdChannels.casualCh.send({ files: embedFilesList, embed: cancelCHEmbed}).catch(console.error);
				}
				else if (draft.mode === 'twos') {
					await cmdChannels.twosCh.send({ files: embedFilesList, embed: cancelCHEmbed}).catch(console.error);
				}
				*/
				await cmdChannels.updatesCh.send({ files: embedFilesList, embed: cancelCHEmbed}).catch(console.error);
				...
				await cmdChannels.modCh.send({ files: embedFilesList, embed: cancelModEmbed}).catch(console.error);
			}
