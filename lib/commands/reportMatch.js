    ...
    if ((userMessage.startsWith('!matchreport ')) || (userMessage.startsWith('!mr '))) {
		...
		if (currentDraft.reportedScores.teamA && currentDraft.reportedScores.teamB) {
			...
			if (
				...
				await currentDraft.captainsClient[otherTeam].send({ files: errorMsgEmbed.embedFiles, embed: errorMsgEmbed.embedMessage}).catch(console.error);
				await currentDraft.captainsClient[currentTeam].send({ files: errorMsgEmbed.embedFiles, embed: errorMsgEmbed.embedMessage}).catch(console.error);
...
async function updateDatabaseWithMatch(matchDraft,isDM,adminAuthor) {
	...
	let embedFilesList = [];
	const embedThumb = new Discord.MessageAttachment('./thumbnails/reportMatch.png', 'reportMatch.png'); //from: https://images.emojiterra.com/twitter/512px/1f3c6.png
	...
	if (matchDraft.mode === 'scrims') {
		for (const player of matchDraft.teamACheckedIn) {
			...
			await playerClient.send({ files: embedFilesList, embed: mrEmbed});
		}
	...
	if (isDM) {
		if (matchDraft.mode === 'casual') {
			await cmdChannels.casualCh.send({ files: embedFilesList, embed: mrEmbed}).catch(console.error);
		}
		else if (matchDraft.mode === 'league') {
			await cmdChannels.leagueCh.send({ files: embedFilesList, embed: mrEmbed}).catch(console.error);
		}
	}
	*/
	
	await cmdChannels.updatesCh.send({ files: embedFilesList, embed: mrEmbed}).catch(console.error);
	...
	if (adminAuthor) {
		...
		await cmdChannels.modCh.send({ files: embedFilesList, embed: mrEmbed}).catch(console.error);
