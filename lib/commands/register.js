...
import { createPlayer, getPlayerFromUsernameQuery } from '../queries/index.js';
import cfg from '../../config.js';
import { logger } from '../index.js';
import Discord from 'discord.js';
import errorMsg from '../scripts/errorMessage.js';

function Player(discordId, username, casualRating, casualSigma, twosRating, twosSigma, foursRating, foursSigma) {
        ...
        if (enforceWordCount(userMessage, 2)) {
			...
			try {
				...
				if (errStr.includes("Field name = username")) {
					...
					console.log(
						`The following error was raised when trying to create new player in player database for username: ${newPlayer.username}, userID: ${newPlayer.userID}.`
					);
					console.log(err);
