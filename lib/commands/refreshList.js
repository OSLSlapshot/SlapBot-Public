...
import { writeSelectList, writeLeagueList, writeBanList, writeTipList, refreshCosmeticDtb } from '../queries/index.js';
    ...
    if ((userMessage.startsWith('!refreshlist')) || (userMessage.startsWith('!rl'))) {
		if (enforceWordCount(userMessage, 2)) {
			...
			if (isMod(msg.author.id)) {
				const allowed_inps = ['select','sel','league','l','ban','b','servertip','st','cosmetics','c'];
				if (allowed_inps.includes(getWord(userMessage,2))) {
					...
					else if ((getWord(userMessage,2) == 'cosmetics') || (getWord(userMessage,2) == 'c')) {
						await refreshCosmeticDtb();
						rlEmbed.author.name = `Cosmetic library was refreshed.`;
					}
