	...
	for (const dir of datesDirs) {
		...
		if (playerIdx !== -1) {
			...
			//if (((PlayerData[0][4] === 'CasualSigma') || (PlayerData[0][4] === 'OpenSigma')) && (parseFloat(PlayerData[playerIdx][4]) !== cfg.trueskill.casualInitTS.initialSigma)) {	//checking to see if any matches played
				...
				const CasualEOSRatingsColIdx = getColNameIdx(PlayerDataColNames, ['CasualRating']);
...
const getVersusCareerInfo = async (discordId, username) => {
	...
	if (playerIdx !== -1 && player2Idx !== -1) {
		...
		/*
		const CasualRatingColIdx = getColNameIdx(PlayerDataColNames, ['CasualRating','OpenRating']);
		const CasualSigmaColIdx = getColNameIdx(PlayerDataColNames, ['CasualSigma','OpenSigma']);
		const TwosRatingColIdx = getColNameIdx(PlayerDataColNames, ['TwosRating']);
		const TwosSigmaColIdx = getColNameIdx(PlayerDataColNames, ['TwosSigma']);
		const FoursRatingColIdx = getColNameIdx(PlayerDataColNames, ['FoursRating']);
		const FoursSigmaColIdx = getColNameIdx(PlayerDataColNames, ['FoursSigma']);
		const CasualWinsColIdx = getColNameIdx(PlayerDataColNames, ['NumCasualMatchesWon','NumOpenMatchesWon']);
		const CasualLossesColIdx = getColNameIdx(PlayerDataColNames, ['NumCasualMatchesLost','NumOpenMatchesLost']);
		const TwosWinsColIdx = getColNameIdx(PlayerDataColNames, ['NumTwosMatchesWon']);
		const TwosLossesColIdx = getColNameIdx(PlayerDataColNames, ['NumTwosMatchesLost']);
		const FoursWinsColIdx = getColNameIdx(PlayerDataColNames, ['NumFoursMatchesWon']);
		const FoursLossesColIdx = getColNameIdx(PlayerDataColNames, ['NumFoursMatchesLost']);
		
		player1 = new Player(
			PlayerData[playerIdx][PlayerIDColIdx],
			PlayerData[playerIdx][DiscordIDColIdx],
			PlayerData[playerIdx][UsernameColIdx],
			parseInt(PlayerData[playerIdx][CasualRatingColIdx]),
			parseFloat(PlayerData[playerIdx][CasualSigmaColId]),
			parseInt(PlayerData[playerIdx][TwosRatingColIdx]),
			parseFloat(PlayerData[playerIdx][TwosSigmaColIdx]),
			parseInt(PlayerData[playerIdx][FoursRatingColIdx]),
			parseFloat(PlayerData[playerIdx][FoursSigmaColIdx]),
			parseInt(PlayerData[playerIdx][CasualWinsColIdx]),
			parseInt(PlayerData[playerIdx][CasualLossesColIdx]),
			parseInt(PlayerData[playerIdx][TwosWinsColIdx]),
			parseInt(PlayerData[playerIdx][TwosLossesColIdx]),
			parseInt(PlayerData[playerIdx][FoursWinsColIdx]),
			parseInt(PlayerData[playerIdx][FoursLossesColIdx]),
			null,
			null,
			null
		);
		player2 = new Player(
			...
			parseInt(PlayerData[player2Idx][CasualRatingColIdx]),
			parseFloat(PlayerData[player2Idx][CasualSigmaColId]),
			parseInt(PlayerData[player2Idx][TwosRatingColIdx]),
			parseFloat(PlayerData[player2Idx][TwosSigmaColIdx]),
			parseInt(PlayerData[player2Idx][FoursRatingColIdx]),
			parseFloat(PlayerData[player2Idx][FoursSigmaColIdx]),
			parseInt(PlayerData[player2Idx][CasualWinsColIdx]),
			parseInt(PlayerData[player2Idx][CasualLossesColIdx]),
			parseInt(PlayerData[player2Idx][TwosWinsColIdx]),
			parseInt(PlayerData[player2Idx][TwosLossesColIdx]),
			parseInt(PlayerData[player2Idx][FoursWinsColIdx]),
			parseInt(PlayerData[player2Idx][FoursLossesColIdx]),
			null,
			null,
			null
		);
