...
/*
function getColNames(arr) {
	//return column names from data array
	console.log(arr[0]);
	return arr[0]
}
*/
...
const getPlayerCareerFromDiscordIdQuery = async (discordId) => {
	...
	for (const dir of datesDirs) {
		...
		if (playerIdx !== -1) {
			let CasualSigmaColIdx = PlayerDataColNames.indexOf('CasualSigma');
			CasualSigmaColIdx = CasualSigmaColIdx === -1 ? PlayerDataColNames.indexOf('OpenSigma') : CasualSigmaColIdx;
			if ((CasualSigmaColIdx !== -1) && (parseFloat(PlayerData[playerIdx][CasualSigmaColIdx]) !== cfg.trueskill.casualInitTS.initialSigma)) {
			//if (((PlayerData[0][4] === 'CasualSigma') || (PlayerData[0][4] === 'OpenSigma')) && (parseFloat(PlayerData[playerIdx][4]) !== cfg.trueskill.casualInitTS.initialSigma)) {	//checking to see if any matches played
				...
				let CasualWinsColIdx = PlayerDataColNames.indexOf('NumCasualMatchesWon');
				CasualWinsColIdx = CasualWinsColIdx === -1 ? PlayerDataColNames.indexOf('NumOpenMatchesWon') : CasualWinsColIdx;
				let CasualLossesColIdx = PlayerDataColNames.indexOf('NumCasualMatchesLost');
				CasualLossesColIdx = CasualLossesColIdx === -1 ? PlayerDataColNames.indexOf('NumOpenMatchesLost') : CasualLossesColIdx;
				let CasualEOSRatingsColIdx = PlayerDataColNames.indexOf('CasualRating');
				CasualEOSRatingsColIdx = CasualEOSRatingsColIdx === -1 ? PlayerDataColNames.indexOf('OpenRating') : CasualEOSRatingsColIdx;
			...
			const FoursSigmaColIdx = PlayerDataColNames.indexOf('FoursSigma');
			if ((FoursSigmaColIdx !== -1) && (parseFloat(PlayerData[playerIdx][FoursSigmaColIdx]) !== cfg.trueskill.foursInitTS.initialSigma)) {
			...
			const TwosSigmaColIdx = PlayerDataColNames.indexOf('TwosSigma');
			if ((TwosSigmaColIdx !== -1) && (parseFloat(PlayerData[playerIdx][TwosSigmaColIdx]) !== cfg.trueskill.twosInitTS.initialSigma)) {
