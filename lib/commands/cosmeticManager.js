...
class CosmeticManager {
	...
	async generateCosmeticManager() {
		...
		const cosmeticsFilter =  (i) => {
			...
			if ((this.isMessageInput) && (this.inputCmd.author.id !== i.user.id)) {
		...
		this.cosmeticsMsg = await this.inputCmd.editReply({files: currEmbed.embedFiles, embeds: [currEmbed.embedMessage], components: currComponents, ephemeral: true, allowedMentions: { repliedUser: false}})
			.then( async (cMsg) => {
				const cosmeticsFilter =  (i) => {
					...
					if ((isMessageClass(this.inputCmd)) && (this.inputCmd.author.id !== i.user.id)) {
						i.reply({content: "This message is not for you!", ephemeral: true});
						return false;
					}
	...
	generateCosmeticTypeEmbed() {
		...
		for (let i = startIdx; i < (startIdx + this.cosmetics_per_page/2); i++) {
			...
			col1val += `${shopTrackerController.cosmeticRarityEmojiMap[currCosmetic.rarity_rank]} ${currCosmetic.name} ${currCosmetic.has_variants ? shopTrackerController.variantsEmoji : ''}${currCosmetic.season ? '🎟️':''}\n`;
		}
		...
		for (let i = startIdx+this.cosmetics_per_page/2; i < (startIdx + this.cosmetics_per_page); i++) {
			...
			const currCosmetic = cosmeticList[i];
			col2val += `${shopTrackerController.cosmeticRarityEmojiMap[currCosmetic.rarity_rank]} ${currCosmetic.name} ${currCosmetic.has_variants ? shopTrackerController.variantsEmoji : ''}${currCosmetic.season ? '🎟️':''}\n`;
		}
