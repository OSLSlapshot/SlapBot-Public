import { getVersusInfo, getVersusCareerInfo } from '../queries';
...
async function versus(msg) {
	...
	if (versusInfo) {
		...
		const userAvatarURL = (await bot.users.fetch(versusInfo.Players.Player1.discordId).catch(console.error)).displayAvatarURL();
		const user2AvatarURL = (await bot.users.fetch(versusInfo.Players.Player2.discordId).catch(console.error)).displayAvatarURL();
