...
import isMessageClass from '../utls/isMessageClass.js';
	...
	if (isMessageClass(cmd_call)) {
		...
		if ((userMessage) && (commandCalls.includes(userMessage[0].toLowerCase()))) {
			...
			*/
			...
			let helpClass = new Help(cmd_call);
	...
	*/
	...
	async generateHelp() {
		...
		if (this.isMessageInput) {
			...
			this.helpMsg = await this.inputCmd.editReply({files: currEmbed.embedFiles, embeds: [currEmbed.embedMessage], components: currComponents, ephemeral: true, allowedMentions: { repliedUser: false}, fetchReply: true});
			this.helpInteraction = this.inputCmd;
		}
		...
		const helpCollector = this.helpMsg.createMessageComponentCollector({ filter: helpFilter, idle: 60000 });
		...
		try {
			helpCollector.on('collect', async i => {
				...
				*/
				...
				await i.editReply({files: newHelp.embedFiles, embeds: [newHelp.embedMessage], components: newHelpButtons, ephemeral: true});
				
				this.helpInteraction = i;
		...
		helpCollector.once('end', async (collected,reason) => {
			if (reason === 'idle') {
				if (this.isMessageInput) {
					const helpMsgComponents = this.helpMsg.components;
					for (const r of helpMsgComponents) {
						for (const b of r.components) {
							b.data.disabled = true;
						}
					}
					await this.helpMsg.edit({components: helpMsgComponents, ephemeral: true});
				}
				else {
					await this.helpInteraction.deleteReply();
				}
			}
		});
