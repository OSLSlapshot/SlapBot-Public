...
import {  getCosmeticNotificationsQuery, updateCosmeticLibraryQuery } from '../queries/index.js';
	...
	async runTracker(scheduledCall = false) {
		...
		if (scheduledCall) {
			...
			for (const cosmetic of all_cosmetics) {
				...
				const notif_dtb = await this.readCosmeticNotificationsDtb();
				
				if ((notif_dtb[cosmetic.type]) && (notif_dtb[cosmetic.type][cosmetic.key])) {
					for (const user of notif_dtb[cosmetic.type][cosmetic.key]) {
						try {
