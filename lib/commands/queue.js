...
async function queue(msg) {
	...
	if ((userMessage === '!lockqueue') || (userMessage === '!lockq')) {
		if (isMod(msg.author.id)) {
			queueBlocked = 1;
			let embedFilesList = [];
			const embedThumb = new Discord.MessageAttachment('./thumbnails/lockQueue.png', 'lockQueue.png'); //from:
			embedFilesList.push(embedThumb);
	...
	if ((userMessage === '!unlockqueue') || (userMessage === '!unlockq')) {
		if (isMod(msg.author.id)) {
			queueBlocked = 0;
			let embedFilesList = [];
			const embedThumb = new Discord.MessageAttachment('./thumbnails/unlockQueue.png', 'unlockQueue.png'); //from:
			embedFilesList.push(embedThumb);
