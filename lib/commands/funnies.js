...
function coinToss(msg) {
	...
	if (userMessage == '!toss') {
		...
		let embedFilesList = [];
		const embedThumb = new Discord.MessageAttachment('./thumbnails/coinToss.png', 'coinToss.png'); //from: https://www.clipartkey.com/mpngs/m/16-165801_coin-clipart-free-transparent-coins-png.png
		embedFilesList.push(embedThumb);
...
function rollDice(msg) {
	...
	if (userMessage == '!dice') {
		...
		let embedFilesList = [];
		const embedThumb = new Discord.MessageAttachment('./thumbnails/rollDice.png', 'rollDice.png'); //from: https://lh3.googleusercontent.com/proxy/HJ8MzoXnVcItRvuIC3OM5LolwCiiZCfoBb2BzYv1EJDFScTBQ8rr1ZXxtunyaKBA_ArL6641RD0BIivFdfuiX-xbHKXoUBQaoTe3mYoAmz2
		//const embedThumb = new Discord.MessageAttachment(`./thumbnails/dice_rolls/${rollNum}.png`, `${rollNum}.png`);
		embedFilesList.push(embedThumb);
...
function rollNumber(msg) {
	...
	if (userMessage == '!roll') {
		...
		let embedFilesList = [];
		const embedThumb = new Discord.MessageAttachment('./thumbnails/rollNumber.png', 'rollNumber.png'); //from: Created on MS Word
		embedFilesList.push(embedThumb);
...
function playingCardDraw(msg) {
	...
	if ((userMessage == '!draw') || (userMessage == '!hitme')) {
		...
		let embedFilesList = [];
		const embedThumb = new Discord.MessageAttachment(`./thumbnails/playing_cards/${card}.png`, `${card}.png`); //from: http://acbl.mybigcommerce.com/52-playing-cards/
		embedFilesList.push(embedThumb);
...
function inspQuote(msg) {
	...
	if (userMessage == '!quote') {
		...
		let embedFilesList = [];
		const embedThumb = new Discord.MessageAttachment('./thumbnails/quote.png', 'quote.png'); //from: Created on MS Word
		embedFilesList.push(embedThumb);
