...
async function displayLB(mode, startPos) {
	...
	if (mode === 'casual') {
		...
		let embedFilesList = [];
		const embedThumb = new Discord.AttachmentBuilder('./thumbnails/lbCasual.png', {name: 'lbCasual.png'}); //from: https://images.emojiterra.com/twitter/512px/1f3c6.png
		embedFilesList.push(embedThumb);

		let lbEmbed = {
			color: 0xd4af37,
			title: `Casual RPUGs Leaderboard (#${startPos} - #${startPos + 14})`,
			thumbnail: {
		...
		let embedFilesList = [];
		const embedThumb = new Discord.AttachmentBuilder('./thumbnails/lbTwos.png', {name: 'lbTwos.png'}); //from: https://images.emojiterra.com/twitter/512px/1f3c6.png
		embedFilesList.push(embedThumb);

		let lbEmbed = {
			color: 0xd4af37,
			title: `Twos RPUGs Leaderboard (#${startPos} - #${startPos + 14})`,
			thumbnail: {
				url: 'attachment://' + embedThumb.name
			},
			description: message,
		...
		result.sort(sortByRatingsFours);
		// Format message
		let count = 0;
		let countLB = 0;
		...
		let embedFilesList = [];
		const embedThumb = new Discord.AttachmentBuilder('./thumbnails/lbFours.png', {name: 'lbFours.png'}); //from: https://images.emojiterra.com/twitter/512px/1f3c6.png
		embedFilesList.push(embedThumb);

		let lbEmbed = {
			color: 0xd4af37,
			title: `Fours RPUGs Leaderboard (#${startPos} - #${startPos + 14})`,
			thumbnail: {
				url: 'attachment://' + embedThumb.name
			},
			description: message,
		...
		let lbEmbed = {
			color: 0xd4af37,
			title: `Scrims RPUGs Leaderboard (#${startPos} - #${startPos + 14})`,
			thumbnail: {
				url: 'attachment://' + embedThumb.name
			},
			description: message,
