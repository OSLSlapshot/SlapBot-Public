...
import { getDraftFromDiscordId, getDraftFromTeamName, removeDraft, isCaptain, getDraftFromIndex } from '../scripts/draftClass.js';
...
class Queue {
	...
	constructor(messageId = null) {
		...
		this.inQueueCheckEnable = false;	//parameter for testing queue- allows users to join multiple times if set to false
	...
	async startQueue() {
		...
		let queueStatusBanner = this.getQueueBanner();
		...
		if (this.queueMsgId) {
			try {
				...
				this.queueStatusMsg = await this.queueStatusMsg.edit({content: '', files: queueStatusBanner, embeds: [queueStatusEmbed], components: queueStatusButtons});
			}
			catch (error) {
				if (error.code === 10008) {
			...
			this.queueStatusMsg = await this.queueCh.send({files: queueStatusBanner, embeds: [queueStatusEmbed], components: queueStatusButtons});
			...
			queueCollector.on('collect', async i => {
				...
				switch (cmdCalled) {
					case 'refresh':
						await i.deferUpdate();
						let followUpMsg = '';
						if ((cfg.idleParams.idleCheckerEnable) && (await this.isInQueue(userId))) {
					...
					case 'help':
						...
						await i.editReply({content: `Not currently implemented- please refer to ${cmdChannels.infoCh} for more info or ask <@183805594690387968> :nerd:`, emphemeral: true});
						break;
					case 'about':
						await i.deferReply({ephemeral: true});
									...
									const kickFromButtons = new Discord.ActionRowBuilder()
										...
										.addComponents(
											new Discord.ButtonBuilder()
												...
												.setLabel(`Kick From ${selectedQueueName.charAt(0).toUpperCase() + selectedQueueName.slice(1)} Queue`)
												//.setEmoji('🏃')
												.setStyle(Discord.ButtonStyle.Primary)
										);
									/*
					...
					case 'scrims':
						...
						if (await this.isInQueue(userId,cmdCalled)) {
							await this.removeFromQueue(userId,cmdCalled);
							const modeName = `${cmdCalled.charAt(0).toUpperCase() + cmdCalled.slice(1)}`;
						...
						if (cmdCalled === 'scrims') {
							...
							else if ((this.allQueues.scrims.length === 1 && LeaguePlayer.teamName === this.allQueues.scrims[0].OSLteam) && (this.inQueueCheckEnable)) {
						...
						const modeName = `${cmdCalled.charAt(0).toUpperCase() + cmdCalled.slice(1)}`;
						logger.log('info', `UserID ${userId} ${i.user.username} (${player.playerID} ${player.username}) joined the ${modeName} RPUGs queue.`);
					...
					case 'current matches':
						...
						i.editReply({files: activeMatchesMsg.embedFiles, embeds: [activeMatchesMsg.embedMessage], emphemeral: true});
						break;
					case 'leave all':
						await i.deferUpdate();
	...
	createQueueComponents() {
		...
		*/
		...
		let queueStatusRow3 = new Discord.ActionRowBuilder()
			...
			.addComponents(
				new Discord.ButtonBuilder()
					...
					.setLabel(`\u200b${padStringToWidth('Scrims (League teams only)',modeButtonWidth,"center")} \u200b`)
					.setEmoji('<:osl:1115348694481510541>')
					.setStyle(Discord.ButtonStyle.Success)
			);
		...
		return [queueStatusRow1, queueStatusRow2, queueStatusRow3, queueStatusRow4, queueStatusRow5];
	}
	...
	*/
	...
	async toggleQueueButtons() {
		...
		for (const r of currQueueButtons) {
			for (const b of r.components) {
				...
				if (queueNames.includes(buttonName)) {
			...
			if (mn === mode) {
				for (const [index, playerObj] of q.entries()) {
					if (player === playerObj.discordId) {
						q.splice(index, 1);
						removed = true;
						const modeName = `${mn.charAt(0).toUpperCase() + mn.slice(1)}`;
						logger.log('info', `UserID ${player} (${playerObj.playerID} ${playerObj.username}) left the ${modeName} RPUGs queue.`);
						break;
					}
				}
				break;
	...
	async addToQueue(player, mode) {
		...
		let poppedQueue = [];
		const queueModeName = `${mode.charAt(0).toUpperCase() + mode.slice(1)}`;
	...
	async executeRefreshOnServer() {
		...
		for (const mn of Object.keys(this.allQueues)) {
			for (const [i,f] of currQueueEmbedFields.entries()) {
				...
				if ((fieldModeName) && (fieldModeName.toLowerCase() === mn)) {
					let numLines;
					for (const m of this.modes) {
	...
	async handleMatchReport(id,score) {
		...
		if (currentDraft.reportedScores.teamA && currentDraft.reportedScores.teamB) {
			...
			if (currentDraft.mode === 'scrims') {
				SREmbed.author.name = `${currCaptainObj.username} (${currCaptainObj.OSLteam}) has reported the match.`;
				SREmbed.description = `Waiting for <@${othCaptainObj.discordId}> (${othCaptainObj.OSLteam}) to also report the match...` + '\n' + `*Please use the :regional_indicator_w: / :regional_indicator_l: buttons in ${this.queueCh} to report.*`;
			}
